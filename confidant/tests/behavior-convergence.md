...
> 验证 confidant 是否「引导到根因 → 必出画像 / 达上限必出小结」，以及 RAG 是否在机制内容前被核对。
...
## 场景 B：达轮次上限未达根因 → 出进展小结

**期望**：12–15 轮内未挖到根因，仍输出「进展小结」（已发现 / 还差什么 / 下一步），对话结束必有产出。

**验证命令**：

```bash
grep -n "进展小结\|轮次预算\|12–15" SKILL.md
```
