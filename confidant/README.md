...
- **v1.2.0（2026-08-10 康复）**：修 `check_readiness.py` CROSS 高频词误判（单字虚词"也/和"→ 短语级交叉信号）+ DONE 危机误判（危机语境"可以了"不算"说完了"，危机词负向排除）+ 格式预检（错误格式 exit 2 明确报错）；危机检测显式留 AI 理由（对话内判断，脚本负向排除兜底）
- v1.0.0 起：对话轴 / 安全轴正交互设计
