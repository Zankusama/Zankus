...
#   step=4  G4 穷尽   提问后：    file=.goal/gate-4.txt（提问记录：必含「目前我听懂了的」小结 + 内部常识轮 + 穷尽性声明；⚠️ 项=0）
...
case "$STEP" in
  ...
  4)
    ...
    # 穷尽性声明（考虑了 N：问了 M/默认 K/以后加 X——没问的被分类安置，不许静默消失）
    if grep -qE '穷尽|问了.*默认|以后可以加|考虑了.*因素' "$FILE"; then
      pass "穷尽性声明存在（考虑因素被分类安置：问了/默认/以后加）"
    else
      fail "提问记录缺穷尽性声明（考虑了 N 个：问了 M/默认了 K/记了 X 以后可以加）——没问的不许静默消失，回步骤 4 补声明"
    fi
    pass "提问穷尽性通过（小结 + 内部常识 + 穷尽性声明 + ⚠️ 清零）"
    ;;
  5)
