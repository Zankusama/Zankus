...
> 打分原则：P0 基本型 5 分（必过）/ P1 期望型 3 分（建议）/ P2 兴奋型 1 分（可选）；层权重 = 项之和，总分 ~127。

---

## 分级：Kano 五类（与评估器 P0/P1/P2 对齐）

| 评估器级别 | Kano 类 | 治疗策略 |
|:--:|:---|:---|
| **P0 基本型** | 基本型 Must-be | **必改**（死规矩 9「只修基本型」= 治疗对象）——影响运行/触发/正确性/安全 |
| **P1 期望型** | 期望型 One-dimensional | 给建议，附在交付里不实施（死规矩 9）|
| **P2 兴奋型** | 兴奋型 Attractive | 记录备选不强制；注意 Kano 迁移（兴奋→期望→基本）|
| — | 无差异型 | 识别即跳过（措辞/格式微调）|
| — | 反向型 | 红线检出即停（过度工程化/强加脚本/抹平风格）|
...
## ③ 骨架层 Skeleton（24 分）· 机制：skill 结构六零件
...
- 判定词：PROGRESS/BLOCKED/.goal/进度 落盘 ≥2 处
